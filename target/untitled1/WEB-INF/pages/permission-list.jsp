...
<body class="hold-transition skin-blue sidebar-mini">
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<ol class="breadcrumb">
				...
				<li><a
					href="${pageContext.request.contextPath}/url/permission-list">权限管理</a></li>
						...
						<div class="table-box">
							...
							<div class="pull-left">
								<div class="form-group form-inline">
									<div class="btn-group">
										<button type="button" class="btn btn-default" title="新建" onclick="location.href='${pageContext.request.contextPath}/url/permission-add'">
							...
							<table id="dataList"
								...
								<thead>
									<tr>
										...
										<th class="sorting_asc sorting_asc_disabled">权限码</th>
	...
	<script>
		...
		function getUserListPageHelper(pageNum,pageSize){
			$.ajax({
				url:$webName + "/permission/permissionSelect",
				type:"post",
		...
		function addTable(pageInfo){
			...
			$.each(pageInfo.list,function(index,permission){
				$("tbody").append("<tr>"+
						"<td>"+ permission.id +"</td>"+
						"<td>"+ permission.name +"</td>"+
						"<td>"+ (permission.type==0 ? "菜单":(permission.type==1 ? "目录":"按钮")) +"</td>"+
		...
		$(document).ready(function() {
			// 激活导航位置
			setSidebarActive("admin-permission");
		...
		$(document).on("click",".btnDetail",function(){
			...
			window.location.href=$webName +"/permission/getPermissionInfoDetailById?id="+id;
		});

		//修改
		$(document).on("click",".btnUpdate",function(){
			var id=$(this).parents("tr").children("td").eq(0).text();
			window.location.href=$webName +"/permission/permissionSelectID?id="+id;
		});

		$(document).on("click",".btnDelete",function(){
			var id = $(this).parents("tr").children("td").eq(0).text();
			if(confirm("确认删除"+id+"资源权限吗")){
				$.ajax({
					url:$webName + "/permission/deletePermission",
					type:"post",
					data:"id="+id,
